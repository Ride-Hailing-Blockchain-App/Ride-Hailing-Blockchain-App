...
contract("RideHailingDispute", (accounts)=>{
    ...
    before(async () => {
        ...
        disputeLibraryContractAddress = await appInstance.disputesLibraryContract();
        disputeLibraryContractInstance = await RideHailingDisputeLibrary.at(disputeLibraryContractAddress);
      });
      ...
      it("Should be able to start a dispute", async() => {
        await disputeLibraryContractInstance.createDispute(driverAccount, "Driver crashed, I want a compensation", 1, false, true, {from: passengerAccount});
      ...
      it("New accounts cannot vote for a dispute due to lack of ratings", async() => {
          await truffleAssert.reverts(disputeContractInstance.voteDispute(0, 1 ,{from:passengerAccount1}));
          await truffleAssert.reverts(disputeContractInstance.voteDispute(0, 2 ,{from:passengerAccount2}));
          await truffleAssert.reverts(disputeContractInstance.voteDispute(0, 1 ,{from:passengerAccount3}));
      });

      it("Cannot return vote deposit to voters when dispute is not resolved", async() => {
        await truffleAssert.reverts(disputeContractInstance.returnAllVoterDeposit(0, {from:accounts[0]}));
      });
      ...
      it("Return vote deposit to voters when dispute is resolved", async() => {
        await disputeContractInstance.returnAllVoterDeposit(0, {from:accounts[0]});
      });
      ...
      it("should allow driver to accept a 2nd ride successfully", async () => {
        ...
        await driverContractInstance.acceptRideRequest(2, { //ride id is 2
          from: driverAccount,
        });
      ...
      it("Should not be able to vote for yourself", async() => {
        await truffleAssert.reverts(disputeContractInstance.voteDispute(1, 1, {from:passengerAccount}));
      ...
      it("Should be able to vote for dispute", async() => {
        ...
        let balance = new BigNumber( await accountsInstance.getAccountBalance({from:passengerAccount1}));
        console.log(balance);
        console.log(oneEth.dividedBy(10).minus(new BigNumber(VOTER_DEPOSIT_AMOUNT)));
        await assert.strictEqual(balance.isEqualTo(oneEth.dividedBy(10).minus(new BigNumber(VOTER_DEPOSIT_AMOUNT))), true, "Voted amount is different");
      })

      it("Ending dispute before reaching minimum", async() => {
        await disputeContractInstance.endVote(1);
        let balance = new BigNumber(await accountsInstance.getAccountBalance({from:passengerAccount1}));
        await assert.strictEqual(balance.isEqualTo(oneEth.dividedBy(10)), true, "Voted amount is different");
      })
      ...
      it("Should allow driver to accept a 3rd ride successfully", async () => {
        ...
        await driverContractInstance.acceptRideRequest(3, { //ride id is 3
          from: driverAccount,
        });
      ...
      it("Should not vote for a dispute that does not exist", async() => {
        await truffleAssert.reverts(disputeLibraryContractInstance.voteDisputeTest(10, 1, {from:passengerAccount2}));
      });
