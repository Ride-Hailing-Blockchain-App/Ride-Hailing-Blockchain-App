...
contract RideDispute {
    ...
    uint public NONCOMPENSATIONDisputed_AMOUNT = MIN_DISPUTE_AMOUNT - TRANSFER_AMOUNT; //for cases whereby there is no need for compensationDisputed, this amount is purely just for voters
    ...
    function createDispute(
        ...
        if (compensationDisputed == true && rideFareDisputed == false) {
            ...
            accountsDataStorage.transfer(NONCOMPENSATIONDisputed_AMOUNT, msg.sender, address(this));
    ...
    function giveInDispute(uint256 disputeId) external {
        ...
        address defendant = disputesDataStorage.getDefendant(disputeId);
        require(msg.sender == defendant, "You are not the dispute's defendant!");
        bool carFareDisputedDispute = disputesDataStorage.getRideFareDisputed(disputeId);
        bool compensationDisputedDispute = disputesDataStorage.getCompensationDisputed(disputeId);

        if (carFareDisputedDispute == true && compensationDisputedDispute == false) {
        ...
        } else if (carFareDisputedDispute == false && compensationDisputedDispute == true) {
            accountsDataStorage.transfer(TRANSFER_AMOUNT, msg.sender, disputesDataStorage.getPlaintiff(disputeId));
        } else if (carFareDisputedDispute == true && compensationDisputedDispute == true) {
            uint256 rideFare = ridesDataStorage.getFare(disputesDataStorage.getRideId(disputeId));
            accountsDataStorage.transfer(rideFare, address(this), disputesDataStorage.getPlaintiff(disputeId));
            accountsDataStorage.transfer(TRANSFER_AMOUNT, msg.sender, disputesDataStorage.getPlaintiff(disputeId));
        }
    ...
    function endVote(uint256 disputeId) external {
        ...
        if (plaintiffVotes >= ((totalVotes * 60) / 100) && totalVotes >= MIN_VOTES_REQUIRED) {
            ...
            if (compensationDisputedDispute == true) {
                ...
                accountsDataStorage.transfer(
                    NONCOMPENSATIONDisputed_AMOUNT,
                    address(this),
                    disputesDataStorage.getPlaintiff(disputeId)
                );
            }
            if (carFareDisputedDispute == true) {
        ...
        uint256 winnerPrize = (NONCOMPENSATIONDisputed_AMOUNT + totalVoterDepositAmount) / winners.length; //transfer remaining amount minus the transfer amount to the correct voters
